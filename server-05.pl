...
$bytes_total = 0;
{
    ...
    if ($data_length > 0){
        ...
        while ($bytes_to_read > 0){
            ...
            $bytes_read = sysread($client_socket, $client_data, $bytes_to_read, length($client_data));
            #$client_socket->recv($client_data_body, $bytes_to_read-1);
